...
class UsersController {
  ...
  async registro(req, res) {
    try {
      ...
      if (exists) {
        return res.status(400).send(response.getResponseCustom(400,  
      ...
      return res.status(201).send(response.getResponseCustom(201, {
      ...
      if (error && (error.messages || error.message)) {
        ...
        if (error.messages) {
          ...
          return res.status(400).send(response.getResponseCustom(400, { data: mensajes }));
        }
        // Si tiene message (string), envíalo directo
        if (error.message) {
          return res.status(400).send(response.getResponseCustom(400, { data: error.message }));
        }
      }
      // Si es otro error, enviar mensaje genérico
      res.status(500).send(response.getResponseCustom(500, { data: 'No se ha podido registrar, intente de nuevo.' }));
    }
  }
}
